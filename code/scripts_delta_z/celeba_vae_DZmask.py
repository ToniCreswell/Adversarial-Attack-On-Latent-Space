...
def get_args():
	...
	parser.add_argument('--outDir', default='../../Experiments_delta_z/celeba_vae_DZmask/', type=str)
...
if __name__=='__main__':
	...
	for e in range(opts.maxEpochs):
		...
		for i, data in enumerate(trainLoader, 0):
			...
			zSwap = z * torch.pow((1 + deltaZ), (- 2 * y + 1)) #if y=0 z * (1 - delta_z), if y=1 z / (1 + delta_z)
			...
			regTerm = torch.norm((deltaZ), p=opts.p)  #want most elements to be "1"
